...
% With this data, we run an LFM with a Gaussian link, and in particular we use the $U \Lambda U^{\top}$ term produced by each yearly model to estimate each states position in a two-dimensional hypersphere. We then evaluate whether their is additional utility gained from using this tensor dependence position, as compared to the similarity of alliance portfolios and UN ideal point similarity measures.
...
To generalize this test of face plausibility, we present all three measures' accounts of six dyadic relationships over time. Additionally, we rescale each of these measures to be between 0 and 1 for comparability, where a 1 indicates that states are farther apart and a 0 that states are closer together.

We first look at three close relationships where we would expect to see similar preferences. Figure \ref{friendly:dyads} depicts the relationships between France and Germany, the US and Israel, and North Korea and China. In all three cases our measure using tensor dependence distance has consistently low values -- nearly $0$ in the case of France and Germany, and China and North Korea, and low but less stable values for the US/Israel relationship. Alliance S-Scores correctly classifies Franco-German and Sino-North Korean friendship, while the US/Israel relationship is characterized as being relatively neutral. The measure based on UN voting is most out of step in terms of characterizing these relationships, with notable divergence in the preferences of many of these pairs.
...
% We finally look at the ability of these measures to capture contentious relationships not involving the US \ref{missing:dyads}. For the Korean relationship, tensor dependence distance better characterizes the relationship as enmity, while S-scores treat the two as having very similar preferences. UN ideal point measure treats their relationships as moderate or neutral. Whereas for Iran and Iraq's relationship S-scores give a consistent close preferences (where data exists), while the tensor dependence metric show more instability, and notable elevation (though not as large as we would expect) during the 1980-88 war. Most interestingly, the UN ideal point measures find Iran and Iraq have a relatively positive relationship when the two states are actually at war.

% \begin{figure}
% 	\centering
% 	\includegraphics[width=.67\textwidth]{plausPlot_3}
% 	\caption{Dyadic relationships between South Korea-North Korea and Iraq-Iran according to the different preference measures. Yearly tensor dependence distances between dyads are denoted by red circles and S-scores by green squares. Note that we have rescale each of these measures to between 0 and 1, where a 1 indicates that states are farther apart and a 0 that states are closer together.}
% 	\label{missing:dyads}
% \end{figure}
