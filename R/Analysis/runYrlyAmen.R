...
foreach(yr = yrs, .packages=c("amen")) %dopar% {
	fit = ameRepNull(
		...
		seed=6886, nscan=3000, burn=1500, odens=1,
		plot=FALSE, print = FALSE
		)
	save(fit, file=paste0(pathResults, 'ameLatSpace/',yr,'_agree3un_defEntAlly.rda'))
}
...
foreach(yr = yrs, .packages=c("amen")) %dopar% {
	...
	save(fit, file=paste0(pathResults, 'ameLatSpace/',yr,'_agree3un_anyAlly.rda'))
}
